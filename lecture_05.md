...
получение элемента контейнера по индексу :clock2:. 
...
элементов в памяти: первый элемент контейнера имеет тот же адрес в памяти, что и сам контейнер 
(то есть, смещение элемента от начала контейнера равно нулю),
...
При этом, :clock2: для строк (и только для строк) оператор членства работает как с отдельными символами, так и с
...
(даже функции: :clock2: об этом мы ещё узнаем).
...
Над списком определены упомянутые нами операции взятия элемента по индексу, среза, проверки членства элемента, с той разницей, 
что каждый элемент рассматривается только поодиночке: 

:exclamation: членство списка в списке означает, что *список-член
входит в список-контейнер как единый элемент*, но 
элементы списка-члена *не являются* сами элементами списка-контейнера. То есть:

```python
assert 1 in [1]
assert not [1] in [1]  # список [1] *не содержится* в списке [1]! Но:
assert [1] in [[1]]  # список [1] *содержится* в списке, содержащем список [1]
```
