...
### Повторение терминов из пятой лекции
...
Наиболее часто данные операторы используются при проверке, что объект (не) является объектом "ничто" -- `None`:
объект `None` в любой программе на Python только один, и ни один другой объект не является `None`. 
...
[Следующая лекция](./lecture_07.md) [Оглавление](./README.md) [Предыдущая лекция](./lecture_05.md) 
