...
utf-8, каждый символ строки, не относящийся к базовой части ASCII (ASCII &mdash; это однобайтовая кодировка, содержащая 128
...
При этом строки юникод в какой-то мере совместимы с обычными: так, если обычная строка содержит только ASCII-символы, 
 то её можно сравнивать со сторками unicode, добавлять друг к другу &mdash; 
 тип результата этих операций будет юникод &mdash; и проверять наличие подстроки:
...
```
Если же хоть один символ в обычной строке &mdash; не-ASCII, то получим ошибку UnicodeDecodeError:
```python
# coding: utf-8
...
Для преобразования символов в строке из текстового вида в числовое представление, равное индексу символа в кодировке,
  используется функция `ord()`. Для обратного преобразования &mdash; функция `chr()` для обычных строк и функция `unichr()` 
  для строк юникод.
...
Одно из основных отличий списка от строк &mdash; то, что данные в списке могут быть разных типов. Но у списка есть ещё одно крайне
...
Операция взятия элемента по индексу для строки и списка работает одинаково: возвращает элемент. Но для списка также
 определена и операция "установить (присвоить) элемент по индексу", и записывается она совершенно логично &mdash; как присваивание значения
...
и они появятся в одной строке с разделением пробелами. Однако следующий `print` опять начнёт с новой строки &mdash; потому, что
...
В лекции 5 и 5а мы познакомились с контейнерными типами данных: строки, списки, кортежи &mdash; и умеем брать элемент по индексу,
 делать срез, а также суммировать контейнеры; узнали, что
строковые типы `str` и `unicode`  отличаются тем, что считается их элементом (байт или символ unicode); узнали, что бывают
неизменяемые объекты и изменяемые; научились автоматизировать проверки результатов программы с помощью утверждения `assert`;
исследовали цикл `for`.

## Задания

1. В Python существует встроенная функция `reversed()`, которая принимает аргумент-контейнер и возвращает также контейнер,
    но с обратным порядком элементов:
     
        >>> reversed([1, 2, 3])
        [3, 2, 1]
     
     Написать аналогичную функцию и проверить, что результат идентичен встроенной, с помощью
    `assert`.

2. В Python существует встроенная функция `sorted()`, которая принимает аргумент-контейнер и возвращает также контейнер,
    но с порядком элементов, отсортированным по возрастанию:
      
        >>> sorted([1, 8, 3, 2])
        [1, 2, 3, 8]
      
    Написать аналогичную функцию и проверить, что результат идентичен встроенной, с помощью
    `assert`. Алгоритм сортировки можно использовать самый простой: последовательное сравнение элемента с оставшимися.

[Следующая лекция](./lecture_06.md) [Оглавление](./README.md) [Предыдущая лекция](./lecture_05a.md) 
