...
* :point_: &mdash; указывает на небольшое отступление от темы или уточнение.
...
Как видно из псевдокода, у нас появляется вложенная логическая структура if-else во втором блоке кода (else-statement).
...
Количество блоков `elif` в структуре не ограничено. В ней может также содержаться и блок `else`, который будет вычислен в том случае, если
ни одно из условий в блоках `if` или `elif` не выполняется.

Блок `else` является так же, как и в простом if-else утверждении, необязательным, 
