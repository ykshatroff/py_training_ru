...
Как и в математике, функция - это сущность, которая может принимать значения и возвращать результат. Возьмём, например, линейную функцию: 
...
Как создать собственную функцию на Python, мы рассмотрим чуть ниже, а сейчас возьмём другую математическую функцию - абсолютной величины числа.
(Напомним, эта функция возвращает x, если он больше или равен 0, или -х, если он меньше нуля, то есть её результат &mdash; всегда неотрицательное число.)
...
Сначала идёт идентификатор функции, далее в круглых скобках - её *параметры* (или *аргументы*), количество которых определяется 
самой функцией (точнее, её *записью* при определении). Функция `abs(x)`, например, принимает один аргумент.

:exclamation: Аргументом функции может быть любое выражение, при этом собственно вызов функции также является выражением. 

:bulb: По соглашению, вызов функции в Python записывается так: идентификатор функции, далее без пробела &mdash; открывающая скобка,
без пробела &mdash; начало выражения (в котором могут быть пробелы) и после конца выражения также без пробела закрывающая скобка.
...
В качесте примера функции с несколькими аргументами приведём полезные функции `min()`, `max()` и `sum()`.
Они вычисляют соответственно минимальный из переданных аргументов, максимальный и сумму. Количество аргументов этих функций может быть любым, от двух :clock2: до 256.
...
Ещё одна полезная функция в Python &mdash; возведение в степень: 

```python
pow(x, y)
```

возведёт x в степень y. Впрочем, обычно в Python
пользуются оператором возведения в степень: `**`

```python
x ** y  # Эквивалентно pow(x, y)
```
