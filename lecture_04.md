...
### Типы выражений с логическими операторами
...
они всегда сохраняют тип выражений-операндов, в отличие от `not` -- выражение, начинающееся с `not`, 
...
`0 or 1` -- `1`, а не `True`; `5 and 0 or None` -- `None` и т.п.; выражение `not 0`, однако, будет равно `True`, `not 1` -- `False`,
...
print int(-2.5)  # -2 -- отбрасывает дробную часть в сторону 0
print int("2")  # 2 -- извлекает число из строки
print float(2)  # 2.0
print float("2")  # 2.0
print float("2.0")  # 2.0

# print int(None)  # TypeError -- неверный тип данных
# print int("test")  # ValueError -- некорректная строка
```
