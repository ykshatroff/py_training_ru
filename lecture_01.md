...
Python &mdash; это не только язык, но и программа-интерпретатор. Любой код на Python выполняется интерпретатором, отсюда следует,
...
Установку Python не будем описывать &mdash; напомним только, что мы будем работать с Python 2, последний выпуск которого &mdash; 2.7.12. 
...
Далее можно ввести какое-нибудь арифметическое выражение &mdash; например, 2+2 и нажать _Ввод_, и Python вывдет ответ 4.
...
Введя `2+2` &mdash; мы ввели так называемое *выражение*. Цифра (число) `2` в этом выражении &mdash; это численный *литерал*, 
символ `+` &mdash; это *оператор*. Таким образом, выражение может состоять из литералов и операторов. Выражение может состоять и 
  из одного литерала: `2` &mdash; это также и полноценное выражение. Выражение имеет некий результат &mdash; в частности, `2+2` имеет результат 4,
...
Целое число и дробное число в Python &mdash; это разные типы данных. Чтобы получить ответ в виде дробного числа 2.5, необходимо, чтобы хотя бы один 
из операндов был дробным числом. (Дробные &mdash; в данном контексте записываемые как десятичная дробь)

В Python нет ограничений на размер целого числа &mdash; в отличие от других ЯП, где объём числа должен укладываться в несколько байт,
 в Python допустим любой размер, например 10 в 100-й или 1000-й степени. Целое число может быть и отрицательным &mdash; если перед литералом
 стоит оператор `-` &mdash; например `-2` . Обратите внимание, `-2` &mdash; это выражение с оператором и литералом. 

С дробными числами ситуация чуть сложнее, но в первом приближении, они также почти не ограничены.
 
Правило операций с числами: если оба операнда целые, то и результат выражения &mdash; целое. Если хотя бы одно число дробное, то результат операции
будет дробным.

## Переменные

Переменная в Python, как и в алгебре, это некое имя, за которым "спрятано" значение. Имя переменной (иначе &mdash; *идентификатор*) должно состоять из
 латинских букв в любом регистре и символов подчёркивания &mdash; в любой последовательности, также после первой буквы или знака подчёркивания
...
разделяя их точкой с запятой `;`, но это противоречит основным принципам Python, хотя и бывает удобно &mdash; впрочем, один из
принципов как раз гласит "практичность важнее безупречности")
 
Переменным можно присвоить как значение литерала, или выражения, состоящего из литералов и операторов, так и значение другой переменной &mdash;
переменная тоже представляет собой допустимое выражение, &mdash; так и выражения с участием переменных.
...
Это ошибка, описание которой в третьей строке начинается со слова `NameError` &mdash; "Ошибка имени: Имя не определено".
