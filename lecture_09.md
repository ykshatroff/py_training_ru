...
(Для последовательностей -- `list`, `str`, `tuple` -- операция `*` означает мультипликацию, то есть "размножение" данной
...
Именованные аргументы можно сочетать с неименованными (порядковыми). Основное правило при этом -- записывать именованные
аргументы после порядковых (нарушение этого правила -- синтаксическая ошибка, `SyntaxError`). При этом порядковые
аргументы распределяются вначале, и затем оставшиеся нераспределёнными аргументы ищутся среди именованных. Таким образом,
первым аргументом будет первый порядковый, вторым -- второй порядковый (если есть), и т.п., и использовать их 
...
и он автоматически получит своё значение. В остальном он ведёт себя как обычный аргумент -- значение может быть взято
...
собой словарь (`dict`), ключом в котором является имя, а значением -- переданное значение. Синтаксически это выглядит
...
Самое интересное -- это то, что мы можем не только объявлять функции с такого рода наборами, но и передавать им аргументы
