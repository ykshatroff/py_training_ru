...
* [Типы и классы в Python](#0KLQuNC_0Ysg0Lgg0LrQu9Cw0YHRgdGLINCyIFB5dGhvbg)
    ...
    * [Всё вокруг &mdash; объекты](#0JLRgdGRINCy0L7QutGA0YPQsyAtLSDQvtCx0YrQtdC60YLRiw)
...
Итак, философски, *объект*, иначе *сущность* &mdash; это воплощение некоего понятия, которое имеет определённые характеристики (свойства)
и определённое поведение. *Тип* &mdash; это абстракция или шаблон для подобных объектов, описывающий их возможные свойства и поведение.
То есть, тип &mdash; это описание класса :clock2: подобных объектов. Рассмотрим, например, автомобили (мы не раз будем обращаться к этой аналогии):
...
Технически, в программировании, *объект* &mdash; это некая область в памяти, заполненная по определённому его *типом* шаблону набором
характерных свойств и поведенческих функций, называемых методами.

Наличие объектов можно отметить во многих языках программирования, а типов &mdash; пожалуй, во всех, начиная от ассемблера. Однако,
...
:star: *Наследование* &mdash; это возможность определить новый тип данных на основании уже существующего (родительского), 
...
:star: *Полиморфизм* &mdash; это  возможность работать
  с объектом такого нового типа как с объектом родительского.
  
:star: *Инкапсуляция* &mdash; это объединение свойств и поведения в одной сущности объекта, 
...
(следующий уровень в иерархии типов &mdash; появляется характеристика "количество колёс"), далее 
(или впереди, порядок в данном случае для иллюстрации) идут свойства: тип кузова, тип двигателя, и т.п., но всё это
в общем не меняет того, что руль и 2-3 педали дают возможность управлять любой машиной примерно
 одинаковым способом (полиморфизм). Инкапсуляция же &mdash; это наличие относительно
 универсального *способа управления* автомобилем, несмотря на такие свойства, как тип или объём двигателя, 
 тип кузова или цвет.
 
:star: Этот способ называется *"интерфейс"*. Интерфейс &mdash; совокупность свойств и поведения, служащих для реализации
 определённой задачи.
...
Как мы выяснили, тип &mdash; это абстракция или шаблон. Выше было особенно отмечено слово "класс": в терминах ООП, абстракции, 
описывающие
свойства и/или поведение, обычно именуются *классом*, отчасти потому, что класс &mdash; это более узкое обобщение, несущее 
более детальные и специализированные характеристики, чем тип, и даже, возможно, общие для разных типов. 
Продолжая аналогии с машинами &mdash; к типу "легковая машина" может принадлежать и машина гоночного класса, и
...
компьютера. Так, тип данных "целое число" &mdash; это совокупность объектов, над которыми компьютер осуществляет операции из
...
:point_up: Тип данных "Объект" &mdash; это абстракция, определяющая свойства и поведение, общие для всех объектов. Таким
...
### Всё вокруг &mdash; объекты

Язык Python, несмотря на свою поддержку различных парадигм программирования &mdash; структурной, императивной, функциональной &mdash; 
...
будь то число, строка, словарь, функция &mdash; является объектом. Сама сущность типа, то есть,
сущность, хранящая в себе информацию о свойствах и поведении объектов этого типа,
также является объектом в Python. Наверно, самый интересный вопрос &mdash; а как же так, что объект имеет тип, который в свою очередь является
...
из типа, при создании объекта) и пользоваться другими полезными эффектами ООП.
...
```
Получается, тип сущности `type` &mdash; тоже `type`, но при этом сущность  `type` принадлежит *и* к типу `object`. А тип `object`
...
* Полиморфизм: 
    * Любой объект типа `Type2`, производного от типа `Type1`, является также сущностью типа `Type1`, как следствие,
      любой объект является сущностью типа `object`.
    * Как следствие предыдущего утверждения, все объекты могут быть переданы в качестве параметра функции `type()`, которая возвращает значение в
      зависимости от типа объекта. 
* Инкапсуляция: 
    * реализация механизма обмена данными между объектами не принципиальна, коль скоро этот обмен производится
...
В терминологии Python, *тип* &mdash; это всякий шаблон, определяющий интерфейс объекта, а *класс* &mdash; это просто тип, 
который создаётся пользователем (то есть, программистом на Python), в противовес встроенным типам, таким как `int` или `object`. 

Так как интерфейс &mdash; это совокупность свойств и методов объекта, то класс как раз и позволяет задать *свойства* и *методы*
для своих объектов.
 
Синтаксически, класс определяется через указание ключевого слова `class`, названия класса и при необходимости, в скобках,
 перечисления типов, от которых класс наследуется, после чего следует специальный блок кода, который может содержать 
 одно или несколько утверждений присваивания (устанавливающих свойства) либо определений функций (устанавливающих методы).
 При этом других типов утверждений (циклов, блоков `try-except`) не допускается, за исключением утверждения `pass`,
  позволяющего создать "пустой" класс :clock2:.
...
существуют две разновидности классов &mdash; старые и новые. Старые уже устарели и их крайне не рекомендуется использовать,
...
:star: В Python, *атрибут* объекта &mdash; это другой объект, связанный с первым отношением членства, таким, что операция
...
:star: *Свойство* &mdash; это атрибут, имеющий непосредственное значение (то есть не являющийся методом). 

:star: *Поле* &mdash; это свойство объекта в том виде, в котором оно указывается в определении класса. Поля &mdash; это шаблоны
...
Как следствие того, что класс &mdash; это шаблон объектов, объект получил атрибут `color`, значение которого равно по умолчанию `None`.
...
ключ словаря &mdash; имя атрибута, значение под этим ключом &mdash; значение атрибута. Если у объекта есть  `__dict__`, то
...
собственных атрибутов, далее, если не находит там нужный ключ &mdash; в атрибуты, определённые в классе объекта. 
Если же не нашёл и в классе &mdash; продолжает поиск в родительском классе, затем в прародительском и далее вверх по иерархии.
Так как у любого объекта общий прародитель &mdash; тип `object`, то атрибут, не имеющийся ни в одном из типов иерархии, ни
...
Возникает забавный вопрос: а можно ли поменять поле класса и что будет с объектами этого класса? Ответ &mdash; в духе Python &mdash;
очень логичен.
...
В отличие от довольно многих ЯП, Python позволяет классам иметь не один, а сколько угодно много родительских типов.
...
<a name="oop">
*ООП* &mdash; Объектно-ориентированное программирование